...
def is_bad_extracted_text(text: str) -> bool:
    ...
    # Признаки шапки/подвала сайта ФАС вместо текста статьи
    fas_markers = [
        "о фас россии",
    ...
    if sum(1 for m in fas_markers if m in t) >= 2:
        return True

    # Признаки навигационного меню сайта вместо текста документа.
    # Принцип: если в тексте одновременно присутствуют несколько пунктов меню,
    # характерных для правовых справочных систем, — это не содержательный текст.
    # Маркеры подобраны так, чтобы не пересекаться с реальными правовыми документами.
    nav_markers = [
    ...
    if sum(1 for m in nav_markers if m in t) >= 2:
        return True
